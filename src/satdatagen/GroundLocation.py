...
class GroundLocation:
	...
	def generate_dataset(self, method = 'krag', limit = None, orbit = 'all', elevation_threshold=10., mixing_coeff = 0.8, output_file = None):
		'''
		...
		@param [elevation_threshold]: float that determines the minimum elevation angle for a satellite to be considered overhead. default is 10 degrees
		...
		'''
		return hel._generate_dataset(self.credentials, self.el, self.time_range.times, method = method, limit = limit, orbit = orbit, elevation_threshold=elevation_threshold, mixing_coeff = mixing_coeff, output_file = output_file)
